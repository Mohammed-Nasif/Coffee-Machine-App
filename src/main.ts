...
const userOptions = async (
	...
	const milkChoice = parseInt(<string>milkPicked);
	const espPicked = await espressoQ();
	const espChoice = parseInt(<string>espPicked);
	// If peppermint mocha
	if (mochaObject instanceof PeppermintMocha) {
		const pepPicked = await peppermintQ();
		const pepChoice = parseInt(<string>pepPicked);
...
const main = () => {
	...
	const buildMocha = async () => {
		do {
			...
			menuChoice = parseInt(<string>optionPicked);
			switch (menuChoice) {
				case 0: {
					break;
				}
